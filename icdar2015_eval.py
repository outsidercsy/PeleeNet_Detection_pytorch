...
from data import ICDAR2015Detection, ICDAR2015AnnotationTransform, ICDAR2015_ROOT####
...
parser.add_argument('--icdar2015_root', default=ICDAR2015_ROOT,####
                    help='Location of ICDAR2015 root directory')
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,####top_k is useless
    ...
    for i in range(num_images):
        ...
        if i < 50:
            ...
            debug = False
        if debug:
            imgpath = os.path.join('/workspace2/csy','data','ICDAR2015_dataset', 'ch4_test_images', '%s') 
            im_origin = cv2.imread(imgpath % image_id)

 
            keep_inds = (top_bboxes[image_id][:, 0] > 0.187)
...
########
        ...
        for i in range(gt.shape[0]):
            ...
            obj_struct['difficult'] = '1' if gt[i][4] < 0  else  '0'
